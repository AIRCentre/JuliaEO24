...
# ╔═╡ 2f5d11df-7e18-4394-b9e8-553ae5ecfda2
begin
	using DIVAnd
	using CairoMakie
	using NCDatasets
...
# ╔═╡ 3471af38-c800-4214-ae53-5f70fe2938d0
md"""
# Illustration of DIVAnd
"""

# ╔═╡ 4befcc1a-c3cc-4be5-9c9a-3de0212997ac
md"""
...
1. You should work in groups. __Choose a group name__ and adapt the `groupname` accordingly
2. Please change only the marked code (after __"Your turn !"__ and before __"Do not make any change below"__)
3. You choose:
     * the location of 100 observations (the vectors __`xobs`__ and __`yobs`__ which corresponds to longitude in degrees East and latitude in degrees North)
     * the correlation length __`len`__
     * the uncertainty of the observation __`epsilon2`__
...
6. __You can submit as many test as you wish, only the best result with lowest root mean square (RMS) error is shown__.


First load all modules
"""

# ╔═╡ 338c8fe7-5b33-4b86-83b2-56a72de843de


# ╔═╡ 1f8876f8-ace8-496c-b335-d36060379d94


# ╔═╡ 12adb783-3a56-4b74-be5b-aca85d9f6672


# ╔═╡ 18e3586b-9217-47a4-b346-9b630e1c80fa
md"""
Download the data file if necessary
"""

# ╔═╡ e67cf379-e4f2-45b4-8274-c42eb943928d
begin
...
# ╔═╡ e32a5569-cad4-4e90-9c73-9c2524781417
md"""
Load the data
"""

# ╔═╡ 036a9fc8-f033-492a-9bbc-5cb30d4877c3
begin
...
# ╔═╡ 0d327373-7b94-4ed6-a2f9-876489862000
md"""
The array `mask` is 1 on sea and 0 on land
"""

# ╔═╡ eeb0d409-d2dc-4a75-9238-0ffaab99b953
mask = isfinite.(vfull);

# ╔═╡ 47c8ae2c-89a4-41e2-a438-b7ee1900d0f1
md"""
Remove the zonal average
"""

# ╔═╡ af092176-5b5c-4aba-99e0-cb6674c24389
begin
	v0 = copy(vfull)
	v0[.!mask] .= 0
...
# ╔═╡ 96f03e32-d980-4322-97c8-deaeaccdcf04
md"""
The scale factors (inverse of the resolution)
"""

# ╔═╡ 76175a81-08cf-46fa-a3e1-36e3bfd8ce89
begin
	pm = ones(sz) ./ ((xi[2,1]-xi[1,1]) .* cosd.(yi));
	pn = ones(sz) / (yi[1,2]-yi[1,1]);
end;

# ╔═╡ 6d7366ed-c819-41f4-936f-44904893ee7b
md"""
Number of observations
"""

# ╔═╡ 7b0ae0b0-b3bd-45fa-a79b-0612f4e26836
nobs = 100

# ╔═╡ e14c4dc0-c71b-4a11-8cd7-c8abcd97dc34
md"""
Noise on observations [°C]
"""

# ╔═╡ 22630d4a-f4bb-4f79-9968-2d00c6109c27
noise = 0.5

# ╔═╡ 8a5ebb9e-a101-48dc-a27d-650c823f17ac
md"""
## Your turn !
...
# ╔═╡ 3048639c-be51-48d8-9def-80497e1fd77b
begin
...
# ╔═╡ b9f5af9f-d441-40f7-b61f-7a01abc08fe5
md"""
## Do not make any change below
"""

# ╔═╡ 1bee20cf-a66f-4651-8c08-68d3f529ca93
md"""
Extract the pseudo-observations
"""

# ╔═╡ 9c04e137-49db-4aa1-9899-b29bda12bef1
begin
	xobs2 = mod.(xobs,360)
...
# ╔═╡ 6f3d00d3-4068-4775-a96c-c124272642d2
md"""
Add some noise
"""

# ╔═╡ 803088df-5e31-46e2-9700-c2c4b0012fe0
begin
	Random.seed!(42)
	vobs_perturbed = vobs + noise * randn(nobs)
end;

# ╔═╡ 8619cea7-d770-4626-b38d-849733a167c1
md"""
Make the analysis
"""

# ╔═╡ ed4fb0d3-605f-4530-b72d-8bb9b34c2bdc
vi,s = DIVAndrun(
    mask,(pm,pn),(xi,yi),(xobs2,yobs),
    vobs_perturbed,
    len,epsilon2
);

# ╔═╡ 956df743-8cf5-4785-83ea-f2cb2739823b
md"""
Plot the results
"""

# ╔═╡ d73175a1-26b1-4921-9875-758fcdd0af28
begin
	fig = Figure(size = (800, 600))
	function map!(ax,gridposition,title)
	    xlims!(ax,extrema(lon))
	    ylims!(ax,extrema(lat))
	    ax.title[] = title
	    # land is in grey
	    contourf!(gridposition,xi[:,1],yi[1,:],mask,levels = [0,.5],colormap = [:grey])
	end
	cl = extrema(filter(isfinite,v))
	ax, hm = heatmap(fig[1,1],xi[:,1],yi[1,:],v,colorrange = cl)
	...
	ax, hm = heatmap(fig[3,1],xi[:,1],yi[1,:],vi,colorrange = cl)
	map!(ax,fig[3,1],"Analysis field")
	Colorbar(fig[4,1],hm, vertical = false)
	
	ax, hm = heatmap(fig[3,2],xi[:,1],yi[1,:],vi - v,colorrange = (-3,3),colormap = :RdBu)
	map!(ax,fig[3,2],"Analysis - true field")
...
md"""
Submit the RMS error to the score board:
[https://data-assimilation.net/scores/?game=DIVAnd2024](https://data-assimilation.net/scores/?game=DIVAnd2024). Update the page by reloading it (`Control-R` on most browsers)
"""
...
function submit_results(groupname,rms_velocity)
    ...
    baseurl = "https://data-assimilation.net/scores"
...
begin
	filename_positions = "positions-rms-RMS-$RMS_difference-len-$len-epsilon2-$epsilon2.txt"
	writedlm(filename_positions,[xobs  yobs])
end
...
manifest_format = "2.0"
project_hash = "96e6b8d72f86dc2d2486215d2ca84b89c6d50f8e"
...
git-tree-sha1 = "d217ed97292d4c9a64422d87b3bee4379bc8494e"
...
git-tree-sha1 = "b73c3f2ed354faa121b90650c0a8c09f661ba9dd"
